...
const INIT_COMMAND = `cd ${DEBUGGER_ROOT} && git checkout mc-tracking && git fetch upstream master && git reset --hard upstream/master && cd ..`;
const UPDATE_COMMAND = `cd ${MC_ROOT} && hg pull -u && cd .. && cd ${DEBUGGER_ROOT} && git fetch upstream master && git reset --hard upstream/master && cd ..`;
